...
If you fix the error\footnote{A takeaway from this is that %
you can't rely on implicit multiplication.
If you write {\tt 7x} the 
system is going to think you've created a new (computer) variable whose 
name is ``7x.''}  
%What you really {\em meant} was {\tt 7\{*}x} and that's %
%what you have to actually type!  
something else a little unexpected happens. Nothing!  There's no output\dots
...
\noindent The semicolon is just a way to sneak multiple commands onto a single line.  The difference here (as opposed to the code above) is that the last command isn't the assignment to the variable \verb+y+, it's just \verb+y+ by itself - and that does have a return value, namely the contents of the variable \verb+y+.
	...
	\item Sage has a function called \verb+sum()+ that adds things up. Next time we'll learn how use the help facility to find out how to use a Sage command, but for now, we're just going to tell you:  the \verb+sum()+ function takes four arguments.  The first argument is a formula for the terms of the sum, the second argument is the so-called summation variable, the last two arguments tell where the sum starts and ends. 
